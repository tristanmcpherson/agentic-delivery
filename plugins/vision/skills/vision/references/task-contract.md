...
`.agentic/state/active-task.json` and task-owned worktree markers are derived context caches and are intentionally outside the task contract's authority. Never use them to amend requirements, approve actions, or promote evidence.
