...
- **Advisory reviewers:** inspect the exact candidate for proof gaps, correctness, security, or regressions; remain builder-side and cannot authorize closure.
...
Lifecycle hooks, role profiles, derived active-task files, execution ledgers, and worktree ownership markers are also orchestration aids. They must be project-scoped, redacting, zero-egress by default, and authority-neutral. They cannot create goals, approve actions, mutate Beads authority, sign evidence, merge, or deploy.
...
Closure and delivery are different decisions. `delivered-and-verified` requires a short-lived Ed25519 attestation from a distinct delivery-controller key. The signed binding includes the current closure-status hash, task/contract, immutable candidate, exact target, independently observed deployment ID, explicit approval ID/actor/time, all required protected post-deploy checks, controller/repository/workflow identity, and fingerprints for both delivery and verifier trust keys. Reusing one key for both principals is rejected.
...
Advisory review artifacts bind the exact current run, check, contract, candidate, workspace, config, harness, criteria, compatible surface, and every reviewed artifact hash. Required adversarial cases and cleanup receipts are explicit. A missing lane, changed artifact, excessive retry, stale run, failed verdict, or inconclusive verdict fails the gate. The authority remains `builder-side-advisory`, including when a different model or thread performed the review.
